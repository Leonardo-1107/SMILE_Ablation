...
class ReconCTDataset(Dataset):
    def __init__(self, file_paths, data_root, image_transforms=None, cond_transforms=None, tokenizer=None):
        """ (training only)
        Args:
        ...
        self.ct_name_bdmapid_mapping = pd.read_csv("dataset_xinze2/phase_translation_map.csv")
    ...
    def __getitem__(self, idx):
        ...
        # add protection for broken CTs
        try:
            slice_idx = random.randint(int(z_shape/3), int(z_shape*2/3)) # make sure to be around abdomen
            input_ct_slice = load_CT_sliceh5(input_ct_path, slice_idx=slice_idx)
            input_gt_slice = load_CT_sliceh5(input_gt_path, slice_idx=slice_idx)
            cond_ct_slice = load_CT_sliceh5(cond_ct_path, slice_idx=slice_idx)
            cond_gt_slice = load_CT_sliceh5(cond_gt_path, slice_idx=slice_idx)# use cond image structure as reference
...
class CTDatasetInference(Dataset):    # for a single CT volume
    ...
    def __getitem__(self, slice_idx): # slice_idx will always in order by setting `shuffle=False`
        ...
        if "arterial" in self.id_map[self.id_map["BDMAP Name"]==self.bdmap_id]["Original Name"].item().lower():
            ...
            text_prompt = "A venous CT slice."
        elif "delay" in self.id_map[self.id_map["BDMAP Name"]==self.bdmap_id]["Original Name"].item().lower():
            text_prompt = "A delayed CT slice."
        elif "contrast" in self.id_map[self.id_map["BDMAP Name"]==self.bdmap_id]["Original Name"].item().lower():
            text_prompt = "A non-contrast CT slice."
        else:
